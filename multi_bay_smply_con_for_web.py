...
def build_word_report(result: Dict[str, object]) -> bytes:
    ...
    generated = document.add_paragraph(
        f"報告產生時間：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}　｜　版本：{APP_VERSION}"
    )
...
def clear_analysis_result() -> None:
    old_result = st.session_state.pop("analysis_result", None)
...
def main() -> None:
    ...
    if OPENSEES_IMPORT_ERROR is not None:
        ...
        st.code(
            "python -m pip install -r requirements_vertical_beam_analysis.txt\n"
            "python -m streamlit run streamlit_vertical_beam_analysis.py",
            language="bash",
        )
