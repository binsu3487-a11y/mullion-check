...
def clear_analysis_result() -> None:
    st.session_state.pop("transom_analysis_result", None)
...
def build_piecewise_wind_loads_for_nodes(
    ...
    for index in range(len(node_x) - 1):
        ...
        q_elements[index] = float(np.trapz(sample_q, sample_x) / (x_j - x_i))
    return q_elements
...
def run_analysis(
    ...
    exact_total_wind = float(np.trapz(exact_q, exact_x))
...
def main() -> None:
    ...
    if analyze:
        try:
            ...
            st.session_state["transom_analysis_result"] = result

    stored_result = st.session_state.get("transom_analysis_result")
